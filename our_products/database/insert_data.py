...
BASE_PATH = os.path.join(os.getcwd(), 'our_products', 'common_products')
...
def insert_product(cursor, product):
    insert_query = """
        INSERT INTO products (
            category, product_title, amazon_price, flipkart_price, myntra_price,
            best_price, amazon_url, flipkart_url, myntra_url,
            image_url, website
...
def process_csv(file_name, category, cursor, db):
    ...
    for col in price_columns:
        if col in df.columns:
            ...
            df[col] = df[col].str.extract(r'(\d+\.?\d*)')  # Extract number
    ...
    for _, row in df.iterrows():
        ...
        product_data = {
            ...
            'flipkart_url': row.get('Flipkart URL'),
            'myntra_url': row.get('Myntra URL'),
            'image_url': image_url,
            'website': source
        }
...
def main():
    ...
    # Process all CSV files with categories
    csv_files = [
        ...
        ("common_sunglasses.csv", "sunglasses")
    ]
